  ...
  */
  ...
  collectTopic = (topicId, isCollect) => {
    ...
    topicId ?
      ...
      : openNotificationWithIcon('warning', '收藏失败！请稍后重试!', 'rgba(0,0,0,.85)', '缺少题目id参数', 2)
  }
  ...
  handleTopicsAndReturnNewStateObj = (topics = []) => {
    ...
    topics.forEach((topicType, topicTypeIndex) => {
      ...
      //遇到一个题目类型，创建一个对应的表格对象添加到records中
    ...
    if (isShow) {
      ...
      setTimeout(_ => {
        ...
        if (noScoreTopics.length === 0) {
          ...
          openNotificationWithIcon('warning', `请设置第 ${noScoreTopics.join("、")} 题的分数`, 'rgba(0,0,0,.85)', '', 4)
        }
      }, 0)
    } else {
      this.setState({ combinationAnalysisModalIsShow: isShow })
 ...
 */
  ...
  */
  ...
  toggleEditState = (topic) => {
    ...
    if (scoreTemp === undefined) {
      if (existArr(topic.materialQuestionList)) {//材料下有题的情况
        this.dealMaterialQuestionScore(topic)
        this.setState({
          isShowSingleTopic: true,
        })
      } else {
        topic.isEdit = true
        this.setState({ topics })
  ...
  handleEveryTopicScoreAndReturnNewStateObj = (topicTypeIndex, score) => {
    ...
    if (newScore === '' || myRegExp.checkScoreFormat.test(newScore + "")) {
        ...
        if (index === topicTypeIndex) {
          ...
          questionList && questionList.length > 0 && questionList.forEach(topic => {
            ...
            if (existArr(topic.materialQuestionList)) {//获取材料下子题的id 并处理分数
              ...
              topic.score = newScore * (topic.materialQuestionList.length);//统计材料下所有小题的分数
            } else {
              topic.score = newScore
            }
          })
      ...
      openNotificationWithIcon('warning', '设置失败！', 'rgba(0,0,0,.85)', '您当前输入的分数不合理（小数位后只能保留一位且是0或者5）', 2)
    }
  }
  ...
  */
  handleSingleEveryTopicScoreAndReturnNewStateObj = (serialNumber, score) => {
    ...
    myRegExp.checkScoreFormat.lastIndex = 0
    if (newScore === '' || myRegExp.checkScoreFormat.test(newScore + "")) {
      if (singleTopic && existArr(singleTopic.materialQuestionList)) {//获取材料下子题的id 并处理分数
        singleTopic.score = 0;
      ...
      let isBreak = false;
      for (const topic of topics) {//处理数据 重新渲染
 ...
 */
  dealMaterialQuestionScore = (question) => {
    if (existArr(question.materialQuestionList)) {
      ...
      tempTopic.materialQuestionList.unshift({ isTopicTypeTitle: true, serialNumber: categoryName })
  ...
  confirmPaperBoard = () => {
    const { dispatch } = this.props
    const { topics, totalScore, paperType, permissionVisible } = this.state
    const paperNameObj = document.getElementById('paperNameInput')
    let paperName = paperNameObj && paperNameObj.value;
    if (!paperName) {
      openNotificationWithIcon('warning', `请先设置组卷名称`, 'rgba(0,0,0,.85)', '', 3)
      return
    }
    if (validatingSpecialCharacters(paperName)) {//验证是否有特殊字符
      return;
    }
    if (!paperType) {
      openNotificationWithIcon('warning', `请先设置试题类型`, 'rgba(0,0,0,.85)', '', 3)
      return
    }
        ...
        callback: () => {
          ...
          this.setState(this.handleTopicsAndReturnNewStateObj([]), () => {
            dispatch(routerRedux.replace({//返回我的题组
              ...
              search: queryString.stringify({ paperType: paperType || "1" })
            }))
          })
          //  @ts-ignore
          if (window._czc) {
 ...
 */
  saveScoreSetting = (scoreData = [], isSingle) => {
    ...
    scoreData = scoreData.map((item) => {
      if (item.score == 0 || !item.score) {//判断分数是否为0或者为空
        isHaveScore = true
      }
      return {
        id: item.tempId,
    ...
    if (isHaveScore) {
      openNotificationWithIcon('warning', `分数不能设置为0或为空`, 'rgba(0,0,0,.85)', '', 3)
      if (!isSingle) {
        this.setState({
