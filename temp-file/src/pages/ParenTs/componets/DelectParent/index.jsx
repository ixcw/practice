...
const DeleteTeacher = (props) => {
  ...
  /**
   * 打开删除弹窗
   * @param	params object	家长详情
   */
  const showDeleteTeacher = (params) => {
    if (Array.isArray(params)) {
      // 批量删除
      setTeacherInfo(params.map(item => item.id).join(','))
    } else {
      // 单条删除
      setTeacherInfo(params.id)
    }
    ...
    dispatch({
      ...
      callback: (res) => {
        if (res.result?.code === 200) {
          // 删除家长账号
          dispatch({
            type: namespace + "/deleteFamilyMesApi",
            payload: { familyIds: TeacherInfo},
            callback: (res) => {
              if (res.result?.code === 200) {
                Modal.success({
                  title: "删除成功！",
