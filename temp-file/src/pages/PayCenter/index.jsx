...
@connect(state => ({
    ...
    userMemberList: state[namespace].userMemberList,//我的会员列表
}))
...
*/
    getUserMemberInfo = () => {
        ...
        //获取商品列表
        dispatch({
            type: namespace + '/getUserMemberInfo',
            payload: {},
            callback: (result) => { }
        })
    }
                        ...
                        <div className={styles['paycenter-warp-box']}>
                            <div className={classString}>
                                ...
                                <div className={styles['member-recharge']}>
                                    <PartingLine
                                        title='会员充值'
                                        description={memberUserInfo && memberUserInfo.memberStudyNameAndEndDate ? `${memberUserInfo.memberStudyNameAndEndDate}` : ''}
                                        memberList={userMemberList}
                                    />
                                    {/* `您的会员到期时间: ${memberUserInfo.memberStudyNameAndEndDate}` */}
                                    ...
                                    <div className={styles['goods-list']}>
                                        {
                                            ...
                                            existArr(goodsList) && goodsList.map((item) => {
                                                return (
                                                    ...
                                                    >
                                                        {
                                                            item && item.name.includes('全科') ? <span className={styles['recommended']}>推荐</span>
                                                                : null
                                                        }
                                                        <div>
                                                            {item.name}
                                                        ...
                                                        <div>
                                                            <div className={styles['goods-price']}>¥ <span> {item.price}</span><span className={styles['a-month']}>/月</span></div>
                                                            {
                                                                item.desc ? <p>{item.desc}</p> : null
                                                            }
                                                        </div>
                                                        {
                                                            seletedStudyInfo.studyId == 16 && !item.subjectId ?
                                                                <div style={{ fontSize: '14px' }}>（包含专业课）</div> : null
                                                        }
                                                    </div>
                                                )
                                            })
                                    ...
                                    <div className={styles['instructions']}>
                                        <h4>说明</h4>
                                        <div>
                                            1.会员付费制按照学段科目进行区分，一次付费只能享受该范围内的专享功能；<br />
                                            2.想要购买其他学段或科目的会员，请切换相应学段以及科目进行解锁吧！ <br />
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className={styles['interests-list']}>
                                        {
                                            [
                                                {
                                                    iconfont: 'icon-cuotidaochu',
                                                    title: '错题导出',
                                                    desc: '错题导出并附带相似题<br />错一练三'
                                                },
                                                {
