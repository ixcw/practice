...
import VersionAddTaskModal from '../components/VersionAddTaskModal';
...
@connect(state => ({
    ...
    bookVersionList: state[namespace].bookVersionList,// 版本列表
    versionKnowledgePointsList: state[namespace].versionKnowledgePointsList,// 版本章节知识点列表
    authButtonList: state[Auth].authButtonList,//按钮权限数据
}))

export default class QuestionBankVersionList extends React.Component {
    constructor(props) {
        super(...arguments);
        this.state = {
 ...
 */
    ...
    */
    getTextbookVersionList = (subjectId) => {
        const { dispatch, gradeList, location } = this.props;
        ...
        dispatch({
            type: namespace + '/getTextbookVersion',
            payload: payloadData,
            callback: (result) => {
                if (result && result.length > 0) {
                    let versionArray = query && query.versionId ? JSON.parse(query.versionId) : [];
                    ...
                    this.setState({
                        versionId: JSON.stringify(versionArray),
                    })
...
*/
   ...
   */
    ...
    */
    handleGradeChange = (gradeCode) => {
        ...
        this.replaceSearch({ subjectId: '', versionId: '', knowId: '', p: 1 })
        this.getSubject(gradeCode);
    }
    ...
    */
    handleSubjectChange = (subjectCode) => {
        ...
        this.replaceSearch({ subjectId: subjectCode, versionId: '', knowId: '', p: 1 })
        this.getTextbookVersionList(subjectCode);
    }
   ...
   */
    handleVersionChange = (versionId) => {
        const versionArray = JSON.stringify(versionId)
        this.setState({
            versionId: versionArray,
        })
        this.replaceSearch({ versionId: versionArray, knowId: '', p: 1 })
    ...
    */
    handleKnowledgeChange = (knowId) => {
        this.replaceSearch({ knowId: JSON.stringify(knowId), p: 1 })
    }
...
* */
        ...
        return (
            <Page header={header} loading={!taskLoading}>
                <div className={classString}>
                    <div className={styles['header-option']}>
                        <div className={styles['left']}>
                            ...
                            <div>
                                <label>版本：</label>
                                <Cascader
                                    value={versionId ? JSON.parse(versionId) : []}
                                    fieldNames={{ label: 'name', value: 'id', children: 'childList' }}
                                    options={bookVersionList}
                            ...
                            <div>
                                ...
                                <Cascader
                                    style={{ width: '240px' }}
                                    value={knowId ? JSON.parse(knowId) : []}
                                    fieldNames={{ label: 'name', value: 'id', children: 'childList' }}
                                    options={versionKnowledgePointsList}
                    ...
                    <div style={{ marginTop: 20 }}>
                        {
                            TaskUploadVisible ? <VersionAddTaskModal
